...
public class AbImageUtil {
    ...
    public static Bitmap scaleImg(Context context,Uri uri,int newWidth,int newHeight) {
        ...
        try {
            resizeBmp=BitmapFactory.decodeStream(context.getContentResolver().openInputStream(uri), null, opts);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
        return resizeBmp;
    ...
    public static Bitmap scaleImg(File file, int newWidth, int newHeight) {
        ...
        Logs.Debug("src w:"+srcWidth+" h:"+srcHeight);
        Logs.Debug("newwidth:"+newWidth+ " "+newHeight);
