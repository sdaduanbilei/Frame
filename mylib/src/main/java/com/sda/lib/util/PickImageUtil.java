...
public class PickImageUtil {
    ...
    private static void init() {
        String tem_image_path = "file://" + FileUtil.appSavePathFile(new Date().getTime() + TEM_IMAGE_NAME);
        Logs.Debug("image uri=" + tem_image_path);
    ...
    public static Intent startFCamera( ) {
        init();
        Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);//action is capture
        intent.putExtra(MediaStore.EXTRA_OUTPUT, imageUri);
        return intent;
    ...
    public static Intent startFPhoto() {
        Intent intent = new Intent(Intent.ACTION_GET_CONTENT, null);
        intent.setType("image/*");
        if (aspectX > 0 && aspectY > 0) {
            intent.putExtra("crop", "true");
    ...
    public static Uri handleResult(int requestCode, int resultCode, Intent data) {
        Logs.Debug("requestCode=" + requestCode + " resultCode=" + resultCode + " data=" + data);
        Uri uri = null;
